sffs_MISS <- function(oFdata=oFdata, iFsel=iFsel, ophen=ophen, reps=3000, pvalor=0.05, file1=file1, file2=file2, file3=file3, cl=NULL)
{
			...
			if (pila[[j]]$state=="FORWARD")
					...
					if (length(colsign)>0)	
						...
						if (length(iFsel)<=2){
							if(CheckGroup(oFdata,iFsel, file3, noms=TRUE)==FALSE){
								...
								#Gpvalue<- GroupPvalue_MISS(iFsel, oFdata, ophen, reps=reps)
				...
				if (length(iFsel)<=2)
					...
					if (CheckGroup(oFdata,iFsel, file1)==FALSE)
						...
						# #write(Gpvalue, file3, append=TRUE, ncolumns=10)
						# noms <-names(oFdata)[iFsel]
						# Gpvalue<- GroupPvalue_MISS(iFsel, oFdata, ophen, reps=reps)					noms <- c(noms,Gpvalue)
						#cat(noms, file=file3, append=TRUE)
						#cat("\n", file=file3, append=TRUE)
# 						}
 						pila[[j]]$index<-pila[[j]]$index+1
